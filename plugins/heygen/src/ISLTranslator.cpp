...
static wstring getPrimaryLang(const wstring &lang, bool withScript = false)
{
    if (lang.empty()) {
        return L"en";
    }
...
static wstring Utf8ToUtf16(const std::string &str)
{
...
static IStream* LoadResourceToStream(int resourceId)
{
	...
	HMODULE hInst = GetCurrentModule();
...
ISLTranslator& ISLTranslator::instance()
{
    static ISLTranslator inst;
    return inst;
}

void ISLTranslator::init(const wstring &lang, int resourceId)
{
    setLanguage(lang);
    is_translations_valid = false;
    ...
    if (IStream *pStream = LoadResourceToStream(resourceId)) {
        ...
        for (WORD i = 0; i < stringsMapSize; i++) {
            ...
            for (WORD j = 0; j < localeMapSize; j++) {
                ...
                localeMap[Utf8ToUtf16(localeName)] = Utf8ToUtf16(translationString);
            }
            translMap[Utf8ToUtf16(stringId)] = localeMap;
        }
...
wstring ISLTranslator::tr(const char *str)
{
    wstring translatedStr = Utf8ToUtf16(str);
...
void ISLTranslator::setLanguage(const wstring &lang)
{
    langName = lang;
